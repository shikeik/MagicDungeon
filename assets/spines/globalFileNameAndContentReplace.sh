...
# 替换后的关键词
REPLACE_STR="3.8.99.1"

# 排除路径/文件的正则表达式
# 为了给文件名和内容检索提供不同的排除逻辑，我们使用两个变量。
# 例如图片文件在重命名时不需要跳过，但内容处理时可以跳过。
# 可设置为空以不排除任何内容
EXCLUDE_PATTERN_NAME="/\.git/"            # 仅用于文件名查找/重命名
EXCLUDE_PATTERN_CONTENT="/\.git/|\.png$|\.jpg$"  # 仅用于内容查找

# 模式: name=仅改文件名，content=仅改文件内容，both=同时处理
MODE="content"          # 可选: name, content, both
...
echo "替换: [$REPLACE_STR]"
echo "模式: $MODE"
echo "排除(文件名): $EXCLUDE_PATTERN_NAME"
echo "排除(内容): $EXCLUDE_PATTERN_CONTENT"
echo "空跑: $DRY_RUN"
echo "========================================"
...
find . -depth -not -path "./$script_name" -name "*${SEARCH_STR}*" | grep -vE "$EXCLUDE_PATTERN_NAME" | while read -r file_path; do
    # 获取目录路径和文件名
...
find . -name "$FILE_PATTERN" -type f -not -path "./$script_name" | grep -vE "$EXCLUDE_PATTERN_CONTENT" | while read file; do
# 先检测是否包含关键词
