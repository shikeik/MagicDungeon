...
1 [x] 视觉bug: 
  - [x] 有头发但没有头部, 头发呈环状散步, 预期应该是呈扇形在头顶, 你难道不知道正常人类长什么样吗? 我需要一个正常人面向右侧的形象
    - [x] 人物面朝右
    - [x] 手肘统一后是左, 即关节限位限制在右侧0~180
    - [x] 所有身体部分全部改用空心有厚度形状而不是极细的实心(现在就是这样), 除了头发
2 [x] 我们已经实现了蒙皮场景, 最难的都搞定了, 现在回头补基础骨骼, 模仿spine的功能(和蒙皮也一样): 
  - [x] 有模式切换: 骨骼设置模式, 动画播放模式
