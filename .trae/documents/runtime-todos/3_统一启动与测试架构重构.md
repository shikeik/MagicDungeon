...
- [x] **定义启动模式枚举 `LaunchMode`**
    - [x] 创建 `com.goldsprite.magicdungeon2.config.LaunchMode`
    - [x] 枚举值: `NORMAL` (正常启动), `DIRECT_SCENE` (直接进场景), `AUTO_TEST` (进场景并跑测试)
- [x] **重构 `DebugLaunchConfig`**
    - [x] 移动到 `com.goldsprite.magicdungeon2.config` 包 (如果需要)
...
- [x] **定义统一测试接口 `IGameAutoTest`**
    - [x] 创建 `com.goldsprite.magicdungeon2.testing.IGameAutoTest`
