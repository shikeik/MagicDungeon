...
#### Phase 0.5: 工具链与UI交互优化 (New)
- [ ] **UI 布局修复**:
  - [ ] 修复 `NeonGenTestScreen` 中 `controls.addSeparator()` 无法占满横向的问题。
  - [ ] 优化 Window 样式，移除 title 背景或改用 List 样式，使其更协调。
  - [ ] 引入 `HoverScrollPane` 解决控制面板过长问题，并修复其焦点抢夺 (Focus Snatching) 问题。
- [ ] **预览与交互增强**:
  - [ ] **Live Vector**: 修复 `fixedSize` 失效问题，并确保 `stretch` 选项保持纵横比 (Aspect Fit) 而非强制拉伸。
  - [ ] **Viewport**: 修复拖动 `generateSize` 时绘制被裁切的问题 (需动态调整 FBO/Camera 或绘制逻辑)。
  - [ ] **Auto Refresh**: 实现 30fps 自动重绘 (带 Dirty 检查)，移除手动 Regenerate/Bake 按钮。
