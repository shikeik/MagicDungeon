...
要在外部 IDE（如 IntelliJ IDEA, Android Studio, VSCode）中实现代码补全，本质上就是**让 IDE 知道 `magicdungeon.jar` 和 `gdx.jar` 在哪里**。
...
MyGame/
  ...
  │     ├── magicdungeon.jar
  │     ├── magicdungeon-sources.jar (强烈建议！让用户能看到源码注释)
  │     └── gdx.jar
  ├── Scripts/              <-- [保持] 源码
...
2.  从引擎自身的运行环境（PC端 classpath 或 Android `assets`）中，把 `magicdungeon.jar`, `gdx.jar` 等**复制**到这个 `libs` 文件夹里。
3.  写入 `build.gradle`。
