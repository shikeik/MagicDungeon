...
REM 修改点：
REM 1. 添加了 \assets\
REM 2. 将 \MagicDungeon2\ 修改为 \MagicDungeon2\MagicDungeon2\ (如果是为了排除子目录)
REM    或者如果不改名，必须小心根目录重名问题。
REM 3. 删除了整个字符串末尾的反斜杠，或者在末尾加个空格，防止转义最后一个引号。

set "EXCLUDE_PATTERN=\MagicDungeon2\MagicDungeon2\ \build\ \bin\ \.git\ \target\ \.idea\ \out\ \assets\ \ig这个会被忽略是个bug\"
