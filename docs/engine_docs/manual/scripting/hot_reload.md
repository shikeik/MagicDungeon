...
MagicDungeon 的核心特性是在运行时编译 Java 代码并热加载。

## 跨平台编译器实现

MagicDungeon 包含一个抽象接口 `IScriptCompiler`，在不同平台有不同实现：
...
2.  **匿名内部类**: 大量修改代码可能导致匿名内部类编号变化 ($1, $2)，导致序列化数据无法正确对应（MagicDungeon 的 JSON 序列化器已针对此做了部分防护，跳过匿名类序列化）。
