...
// - 当前已完成(开始时置0): 2
// - 时间精确到分钟
// - 开始时间: 2026-02-16 12:45
// - 结束时间: 2026-02-16 13:25
// - 总耗时: 40分钟
...
## 待检查 (ai可直接忽略不读)
- [x] (待检查) 完善的音乐音效，packs中quilly-crawler有完善的地牢音乐音效，设计一个游戏Audio系统来统一游戏中音乐音效逻辑
    - [x] (待检查) 创建 `AudioAssets` 类: 统一管理音频资源路径与加载。
    - [x] (待检查) 重构 `AudioSystem`: 改为单例模式，集成 `AssetManager`，实现音乐/音效播放接口。
    - [x] (待检查) 集成到游戏场景: `GameScreen` (营地/地牢BGM切换) 和 `MainMenuScreen` (菜单BGM)。
    - [x] (待检查) 映射音效: 将原有程序化音效 (攻击/受击/拾取等) 映射到 Quilly-Crawler 资源包。
    - [x] (待检查) 音量控制: 完善 `SettingsManager` 与 `AudioSystem` 的联动，支持动态调整音量。
